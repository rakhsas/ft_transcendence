...
class Game {
  ...
  collision(player: User | Computer | any, ball: Ball | any){
    player.top = player.y;
   ...
   let result = (ball.right > player.left && ball.top < player.bottom && ball.left < player.right && ball.bottom > player.top);
    console.log(result);
    return result;
  ...
  movePaddle(event: any){
    ...
    this.user.x = event.clientY - rect.top - this.user.height/2;
  ...
  update(){
    ...
    this.computer.y += (this.ball.y - (this.computer.y + this.computer.height/2)) * computerLevel;
    ...
    if(this.collision(player, this.ball))
    {
